...
export const ReportDetails: React.FC = () => {
    ...
    return (
      <div className="p-8 max-w-7xl mx-auto w-full space-y-8 animate-pulse">
        <div className="flex items-center justify-between">
          ...
          <div className="h-10 w-32 bg-[var(--surface-hover)] rounded-lg" />
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-5 gap-5">
          {[...Array(5)].map((_, i) => (
            <div key={i} className="h-28 bg-[var(--surface)] border border-[var(--outline)] rounded-2xl" />
          ))}
        </div>
        <div className="h-64 bg-[var(--surface)] border border-[var(--outline)] rounded-2xl" />
        <div className="h-96 bg-[var(--surface)] border border-[var(--outline)] rounded-2xl" />
      </div>
    );
  }
  ...
  if (error || !project) {
    return (
      <div className="p-8 max-w-md mx-auto mt-20 text-center space-y-4 bg-[var(--surface)] border border-[var(--outline)] p-8 rounded-2xl shadow-xl">
        <AlertTriangle className="w-12 h-12 text-[var(--color-danger)] mx-auto" />
        <h2 className="text-xl font-bold text-[var(--ink)] tracking-tight">Report Unavailable</h2>
        ...
        <div className="pt-2 flex flex-col gap-2">
          <button
            onClick={() => refetch()}
            className="w-full flex items-center justify-center gap-2 bg-[var(--surface-hover)] hover:bg-[var(--outline)] text-[var(--ink)] font-semibold py-2.5 rounded-xl border border-[var(--outline)] transition-colors"
          >
    ...
    <div className="p-8 max-w-7xl mx-auto w-full space-y-8 select-none">
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          <button 
            ...
            className="p-2.5 bg-[var(--surface)] hover:bg-[var(--surface-hover)] rounded-xl border border-[var(--outline)] transition-colors text-[var(--ink-muted)] hover:text-[var(--ink)]"
          >
            <ArrowLeft className="w-5 h-5" />
          </button>
          <div>
            <div className="flex items-center gap-3">
              <h1 className="text-2xl font-extrabold text-[var(--ink)] tracking-tight">Final AI Audit Report</h1>
            ...
            <p className="text-[var(--ink-muted)] text-xs mt-1.5 font-medium">
      ...
      <div className="space-y-4">
        <div>
          ...
          <p className="text-xs text-[var(--ink-muted)] mt-1">Review functional status, latency profiles, and diagnostics across tested schemas</p>
        </div>
